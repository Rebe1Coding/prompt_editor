...
ответа блоком «Может пригодиться». Суб-агент не решает задачу из промпта — он только
прикрепляет полезные ссылки. Если поиск недоступен или Tavily-ключ не задан, блок просто
не добавляется, а основной ответ не страдает.
...
| `search` | ключ Tavily (`tavily_api_key`) и число статей (`max_results`) |
| `storage` | пути к базе, папке результатов, системному промпту и промпту суб-агента |
| `server` | хост и порт |

Системный промпт редактора (`prompts/system_prompt.md`) и промпт суб-агента для поиска
(`prompts/search_query_prompt.md`) перечитываются на каждый запрос — править можно без
...
app/
  ...
  suggester.py # суб-агент: запрос по промпту, поиск, блок «Может пригодиться»
